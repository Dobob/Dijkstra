...
*/
  ...
  if( isEmpty( ) )
      throw underflow_error{"Error: Empty binomial queue!"};
...
int BinomialQueue<Comparable>::findMinIndex( ) const
{
  ...
  for( minIndex = i; i < theTrees_.size( ); ++i )
    if( theTrees_[ i ] != nullptr && theTrees_[ i ]->element < theTrees_[ minIndex ]->element )
		{
			//cout << "theTrees_[ i ]->element: " << theTrees_[ i ]->element.getDist() << endl;
			//cout << "theTrees_[ minIndex ]->element : " << theTrees_[ minIndex ]->element.getDist()  << endl;
...
template <typename Comparable>
void BinomialQueue<Comparable>::percolateUp(BinomialNode * t)
{

	while((t->parent != nullptr) && (t->element < t->parent->element ) )
	{	
		Comparable percolatedItem = t->element;
...
template <typename Comparable>
void BinomialQueue<Comparable>::decreaseKey(Comparable & obj, const DistType & new_key)
{

	if(!hashTable_.contains(obj))
	{		
		return;
	}
	
	(hashTable_[obj]->element).setDist(new_key);


	percolateUp(hashTable_[obj]);
